...
let questions = [
    {
        question: "Kuantum fiziğinde, gözlemcinin sistemi nasıl etkilediğini açıklayan kavram nedir?",
        options: { A: "Süperpozisyon", B: "Örüntüleme", C: "Kuantum Dolanıklığı", D: "Gözlemci Etkisi" },
        correct: "D"
    },
    {
        question: "'Gödel'in Eksiksizsizlik Teoremi' neyi ifade eder?",
        options: { A: "Tüm matematiksel ifadelerin doğrulanabilir olduğunu", B: "Her tutarlı biçimsel sistemin karar verilemez ifadeler içerdiğini", C: "Tüm doğa yasalarının birleşik bir teoride açıklanabileceğini", D: "Zaman yolculuğunun mümkün olduğunu" },
        correct: "B"
    },
    {
        question: "Evrenin genişlemesinin hızlanmasından sorumlu olduğu düşünülen gizemli enerji türü nedir?",
        options: { A: "Kara madde", B: "Kara enerji", C: "Nükleer enerji", D: "Kinetik enerji" },
        correct: "B"
    },
];
...
exports.execute = async (client, message, args) => {
    ...
    const filter = (interaction) => ["A", "B", "C", "D"].includes(interaction.customId) && !interaction.user.bot;
    const collector = sentMessage.createMessageComponentCollector({ filter, time: 2400000 });

    let userAnswers = new Map();

    collector.on("collect", async (interaction) => {
        if (userAnswers.has(interaction.user.id)) {
            return interaction.reply({ content: "❌ Zaten bir cevap verdiniz!", ephemeral: true });
        }

        userAnswers.set(interaction.user.id, interaction.customId);

        await interaction.reply({ content: `✅ Cevabınızı aldık: **${interaction.customId}**`, ephemeral: true });
    });
    ...
    collector.on("end", async () => {
        ...
        const resultEmbed = new MessageEmbed()
            ...
            .addFields(
                { name: "Doğru Cevap Verenler:", value: correctUsers.length > 0 ? correctUsers.join("\n") : "Kimse doğru cevap vermedi." },
                { name: "Yanlış Cevap Verenler:", value: incorrectUsers.length > 0 ? incorrectUsers.join("\n") : "Kimse yanlış cevap vermedi." }
            )
            .setColor("GREEN")
