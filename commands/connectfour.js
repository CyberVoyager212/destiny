...
module.exports.help = {
    name: 'connectfour',
    aliases: ['connect4', 'c4'],
    description: 'Bir kullanıcıyla veya bot ile Dört Bağlantı (Connect Four) oyunu oyna.',
...
module.exports.execute = async (bot, message, args) => {
    ...
    if (!opponent) {
        ...
        if (!args[0] || !['basit', 'kolay', 'normal', 'zor', 'imkansız'].includes(args[0].toLowerCase())) {
            return message.channel.send("Lütfen bir zorluk seviyesi seç: **basit, kolay, normal, zor veya imkansız**");
        }
    ...
    if (!opponent) {
        return message.channel.send("**Lütfen oynayacak bir kullanıcı etiketleyin!**");
    }
    ...
    if (currentGame) return message.channel.send(`**Bu kanalda zaten devam eden bir oyun var!**`);
...
${nums.join(' ')}`);
    ...
    return message.channel.send(winner ? `**Tebrikler, ${winner}! Kazandın!**` : '**Berabere!**');
};
...
function displayBoard(board) {
    return board.map(row => row.map(piece => piece === 'user' ? playerOneEmoji : piece === 'oppo' ? playerTwoEmoji : blankEmoji).join('')).join('\n');
}
