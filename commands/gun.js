...
module.exports.execute = async (bot, message, args) => {
    if (!args[0]) return message.channel.send("**Lütfen bir kullanıcı etiketleyin!**");
    ...
    if (opponent.user.bot) return message.channel.send('**Botlarla düello yapamazsınız!**');
    if (opponent.user.id === message.author.id) return message.channel.send('**Kendi kendinize düello yapamazsınız!**');
    ...
    try {
        ...
        await message.channel.send(`**HIZLI OLUN! \`${word.toUpperCase()}\` YAZIN!**`);
