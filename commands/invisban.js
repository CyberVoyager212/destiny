...
module.exports = {
    name: 'invisban',
    description: 'Kullanıcının tüm rollerini alır ve "Yasaklı" rolü vererek onu görünmez şekilde yasaklar.',
    aliases: ['gizliban', 'silentban'],
    usage: 'k!invisban <@kullanıcı | kullanıcı ID | kullanıcı adı> <sebep>',

    async execute(client, message, args) {
        if (!message.member.permissions.has('MANAGE_ROLES')) 
            return message.reply('❌ Bu komutu kullanmak için **Rolleri Yönet** iznine sahip olmalısın!');

        if (!args[0]) return message.reply('❌ Lütfen yasaklanacak kullanıcıyı **etiketleyin, adını yazın veya ID girin!**');

        let user = message.mentions.members.first() || 
                   message.guild.members.cache.get(args[0]) || 
                   message.guild.members.cache.find(m => m.user.username.toLowerCase() === args[0].toLowerCase());

        if (!user) return message.reply('❌ Kullanıcı bulunamadı! **Doğru bir kullanıcı adı veya ID girdiğinizden emin olun.**');
        ...
        if (!banRole) {
            try {
                ...
                return message.reply('❌ "Yasaklı" rolü oluşturulurken bir hata oluştu!');
            }
        }
        ...
        try {
            ...
            message.channel.send(`✅ **${user.user.tag}** kullanıcısı görünmez şekilde yasaklandı! **Sebep:** ${reason}`);
        } catch (err) {
