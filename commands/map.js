...
exports.execute = async (client, message, args) => {
    ...
    try {
        ...
        const res = await fetch(`https://image.thum.io/get/width/1920/crop/675/noanimate/${site}`);
        const buffer = await res.buffer();

        // Eğer API bir hata döndürdüyse
        if (!buffer || buffer.length < 100) {
            return message.reply("❌ **Görsel oluşturulamadı! Lütfen farklı bir konum deneyin.**");
        }
        ...
        return message.reply(`❌ **Bir hata oluştu:** \`${err.message}\`. Lütfen daha sonra tekrar deneyin.`);
    }
};
