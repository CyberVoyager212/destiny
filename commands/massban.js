...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!users.size) return message.reply('Lütfen yasaklanacak kullanıcıları etiketleyin.');

        let success = 0, failed = 0;
        for (const [id, member] of users) {
            try {
                await member.ban({ reason: `Toplu Yasaklama - Yetkili: ${message.author.tag}` });
                success++;
            } catch (error) {
                failed++;
            }
        ...
        message.channel.send(`✅ **Başarıyla yasaklananlar:** ${success} kişi\n❌ **Başarısız:** ${failed} kişi`);
    },
