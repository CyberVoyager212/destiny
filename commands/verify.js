...
const generateCaptchaImage = (captchaText) => {
  ...
  for (let i = 0; i < numLetters; i++) {
    ...
    const y = baseY + (Math.random() * 2 * verticalDeviation - verticalDeviation);
    positions.push({ x, y });
          ...
          if (customRoleID) {
            try {
              ...
              if (role) {
                ...
                console.log(`Custom role ${role.name} silindi ve doğrulama rolü eklendi.`);
              } else {
...
exports.help = {
  ...
  usage: "doğrulama",
  description:
    "Kullanıcının doğrulama rolü almasını sağlar.",
};
