...
module.exports = async (client, member) => {
    ...
    if (config.inviteTracking) {
        try {
            ...
            if (inviterId) {
                await client.db.set(`inviter_${member.guild.id}_${member.id}`, inviterId);
            }
        } catch (err) {
    ...
    entryMessage = entryMessage.replace(/\$katılım/g, member.joinedAt ? member.joinedAt.toLocaleDateString() : 'N/A');
    ...
    const welcomeChannel = member.guild.channels.cache.get(config.incomingChannel);
    if (welcomeChannel) {
        if (embed) {
    ...
    if (config.otorol) {
        ...
        if ((config.otorol.type === 'bot' && member.user.bot) ||
            (config.otorol.type === 'kullanıcı' && !member.user.bot)) {
            for (const roleId of config.otorol.roles) {
                const role = member.guild.roles.cache.get(roleId);
                if (role) {
                    member.roles.add(role).catch(err => console.error("Otorol eklenemedi:", err));
                }
            }
        }
    }
    ...
    if (config.otoisim) {
        if ((config.otoisim.type === 'bot' && member.user.bot) ||
            (config.otoisim.type === 'kullanıcı' && !member.user.bot)) {
            member.setNickname(config.otoisim.name).catch(err => console.error("Otoisim ayarlanamadı:", err));
        }
    }
};
