...
module.exports = async (client, member) => {
    ...
    exitMessage = exitMessage.replace(/\$katılım/g, member.joinedAt ? member.joinedAt.toLocaleDateString() : 'N/A');

    // Davet takibi: üyenin girişinde kaydedilen davet bilgisini kullanıyoruz
    let inviterText = 'Bilinmiyor';
    if (config.inviteTracking) {
        const inviter = await client.db.get(`inviter_${member.guild.id}_${member.id}`);
    ...
    const goodbyeChannel = member.guild.channels.cache.get(config.outgoingChannel);
    if (goodbyeChannel) {
        if (embed) {
